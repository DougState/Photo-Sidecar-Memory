...
**Active branch:** `feature/taste-discovery-ps-works` — taste discovery pipeline (PSD/PSB style mining). As of 2026-05-21 this branch absorbs both `cursor/*` work branches (dep-pin + CLI flag plumbing + file-handle leak fix; PSD tree/opacity bug fixes) and is merged ahead of `main` by a fast-forward.
...
- **PSD introspection module (`bridge_assist/psd_introspect.py`):** shared lazy reader used by both the style miner and the existing `inferrer.py`. Single source of truth for layer walks, adjustment-kind classification, partial introspection, and per-file timeouts. `_tree_signature` now handles PSDImage roots (recurses into children rather than returning `?`); `_opacity_bucket` distinguishes 0-255 int from 0.0-1.0 float via `isinstance` so opacity=1 (~0.4%) is no longer misclassified as fully opaque.
- **Test suite (`tests/test_taste_discovery.py`):** 16 unit + integration tests covering psd_introspect helpers, chain detection, featurization, and a full mine-styles smoke test on synthetic PSDs. All passing in ~6s on `.venv/bin/python -m pytest tests/ -v`.
- **DECISIONS.md:** project-root architectural-decisions log. Currently one entry (public-mirror strategy: force-pushed sanitized snapshot of private `main` to `DougState/Photo-Sidecar-Memory`). Add new entries on top, append-only.
...
3. **Fix EXIF extraction for D800 Nikon-specific fields.** The sort report currently shows `Unknownmm f/Unknown ISO Unknown` for the MT88026 test image. exiftool integration (commit `0fafe74`) should resolve this, but the Hawaii run needs verification. If EXIF is still missing, check whether exiftool is installed and the ingest code is actually calling it.
4. **Test the taste engine feedback loop end-to-end.** Phase 1 is built (`record`, `record-dir`, `feedback`) but hasn't been tested with real processed files (PSDs from Photoshop, TIFs from Capture One). Need to process a few Hawaii images through the actual NX-D/Photoshop workflow and then run `record-dir` to verify the matcher logic.
5. **Add `--only-unscored` resumption test.** The flag exists in the score command. Verify it correctly skips already-scored images when resuming a partially-completed run (kill mid-run, then resume with `--only-unscored`).
6. **Consider batch/parallel scoring.** At 9 seconds per image, 499 images took 75 minutes. Investigate whether the Anthropic API supports concurrent requests and whether batching 3-5 images in parallel would be safe and faster.
7. **Expand the test suite past taste discovery.** `tests/test_taste_discovery.py` (16 tests, passing) covers the style miner. Other core modules have nothing. Priority targets: `taste_parser.py` (parse/validate taste.md), `tagged_filename()` helper in cli.py, `parse_scores_response()` JSON extraction in score.py, `_confidence_bar()` in report.py. Also add `pytest` to a dev-deps section in `pyproject.toml` (it currently has to be installed manually into `.venv`).
8. **Extend DECISIONS.md.** File now exists at project root with one entry (public-mirror strategy, 2026-05-09). Backfill the rest: single-call multi-channel scoring, Claude > OpenAI for taste scoring, symlinks not copies, per-channel report layout, page-scoped XMP labels, working-dir-at-cwd vs source-folder, size-guard threshold for >1GB PSBs.
...
| 2026-05-21 | Bootstrapped long-running agent harness on `feature/taste-discovery-ps-works` | Added `init.sh`, `claude-progress.txt`, `feature_list.json` (8 features seeded from this file), `docs/history/sessions/`, `.claude/settings.json`, optional warning-only `.git/hooks/pre-commit`. Prepended a session ritual to `CLAUDE.md`; skill-routing block preserved. `./init.sh` reports healthy. Committed as `a18b20f`. |
| 2026-05-21 | Merged both `cursor/*` work branches + `main` into `feature/taste-discovery-ps-works`; fast-forwarded `main` | `cursor/core-application-issues-7e60` brought `numpy` + `scikit-learn` to deps, plumbed `partial_threshold_bytes` and `timeout_seconds` through `mine_styles → phase1_extract` (CLI flags previously silently dropped), and closed a file-handle leak in `styles_report_cmd`. `cursor/psd-tree-and-opacity-923f` fixed `_tree_signature` PSDImage-root recursion and `_opacity_bucket` int-vs-float opacity classification. Also merged `main` to absorb its two divergent commits (`DECISIONS.md` addition, parallel elimstat-removal), resolving a `.gitignore` conflict (kept feature's organized sections, added `*.mpt`). 16/16 tests passing in ~6s after each step. Installed `pytest` into `.venv` (it wasn't there). Then `main` fast-forwarded onto feature. |
...
- **`pytest` is not declared in `pyproject.toml`.** Installed manually into `.venv` on 2026-05-21 (`pip install pytest`). If you recreate the venv, run that again before `python -m pytest tests/ -v`. Adding a `[project.optional-dependencies] dev = ["pytest>=8"]` block is on the todo list.
- **`DECISIONS.md` is on `main` and `feature/taste-discovery-ps-works`.** Currently one entry (public-mirror strategy). Append new entries at the top with date headings; do not edit older entries — supersede them with a new entry.
